        ...
        while (hasMore() && (peek() == '*' || peek() == '+' || peek() == '?' || peek() == '{')) {
            switch (peek()) {
                ...
                case '{':
                    ...
                    for (int i = 0; i < numTwo - numOne + 1; i++) {
                        if (i != 0) {
                            ...
                            segment.addChoice(RegExPrimitive.getRegExPrim(true, false, false));
                        }
                    }
                    tempSeq.add(segment);
                    base = tempSeq;
                ...
                case '?':
                    ...
                    tempChoice.addChoice(RegExPrimitive.getRegExPrim(true, false, false));
    ...
    private RegEx range() {
        ...
        if (peek() == '^') {
            ...
            while (peek() != ']') {
                ...
                char head = ((RegExPrimitive) comp1).getChar();
                data[alph.indexOf(head)] = 0;
                if (peek() == '-') {
                    eat('-');
                    RegEx comp2 = base(false);
                    char tail = ((RegExPrimitive) comp2).getChar();
            ...
            for (int i = 0; i < data.length; i++) {
                if (data[i] != 0) {
                    opt.addChoice(RegExPrimitive.getRegExPrim(alph.charAt(i)));
                }
            }
        } else {
            while (peek() != ']') {
                opt.addChoice(base(false));
                char head = ((RegExPrimitive) opt.fetchChoice(opt.choiceCount() - 1)).getChar();
                if (peek() == '-') {
                    eat('-');
                    opt.addChoice(base(false));
                    char tail = ((RegExPrimitive) opt.fetchChoice(opt.choiceCount() - 1)).getChar();
                    ...
                    for (char i = (char) (head + 1); i < tail; i++) {
                        opt.addChoice(RegExPrimitive.getRegExPrim(i, negate));
                    }
                }
            }
        }
    ...
    private RegEx base(boolean complexAvail) {
        ...
        switch (peek()) {
            ...
            case '.':
                if (complexAvail) {
                    next();
                    return RegExPrimitive.getRegExPrim(false, true, false);
                }
                return RegExPrimitive.getRegExPrim((Character) next());
            case '$':
                if (complexAvail) {
                    next();
                    return RegExPrimitive.getRegExPrim(false, false, true);
                }
                return RegExPrimitive.getRegExPrim(next());
            case '\\':
                next();
                switch (peek()) {
                    case 'n':
                        next();
                        return RegExPrimitive.getRegExPrim('\n');
                    case 'r':
                        next();
                        return RegExPrimitive.getRegExPrim('\r');
                    case 't':
                        next();
                        return RegExPrimitive.getRegExPrim('\t');
                    default:
