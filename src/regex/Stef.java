...
public class Stef implements FSANode {
    private int index = 0;
    private final Map<RegExPrimitive, Set<FSANode>> shiftMatrix = new HashMap<>();
    ...
    //region Constructors
    Stef() {}
    Stef(FSANode next, RegExPrimitive prim) {
    ...
    //region Connections
    @Override
    public void registerConnection(FSANode next, RegExPrimitive prim) {
        shiftMatrix.putIfAbsent(prim, new HashSet<>());
        shiftMatrix.get(prim).add(next);
    }
    @Override
    public Set<RegExPrimitive> possibleInputs() {
        return shiftMatrix.keySet();
    }
    @Override
    public Set<FSANode> possibleTransitions(RegExPrimitive input) {
        return shiftMatrix.get(input);
    }
    @Override
    public Set<FSANode> fetchNext(RegExPrimitive input) {
        if (input.equals(RegExPrimitive.getRegExPrim(true, false, false))) {
            return shiftMatrix.get(input);
        }
        boolean valid = false;
        Set<FSANode> other = new HashSet<>();
        if (shiftMatrix.containsKey(RegExPrimitive.getRegExPrim(false, true, false))) {
            valid = true;
            other.addAll(shiftMatrix.get(RegExPrimitive.getRegExPrim(false, true, false)));
        }
    ...
    public FSANode fetchSingleNext(RegExPrimitive input) {
        if (input.equals(RegExPrimitive.getRegExPrim(true, false, false))) {
        ...
        if (shiftMatrix.containsKey(RegExPrimitive.getRegExPrim(false, true, false))) {
            return shiftMatrix.get(RegExPrimitive.getRegExPrim(false, true, false)).iterator().next();
        }
        return null;
    }
    //endregion
